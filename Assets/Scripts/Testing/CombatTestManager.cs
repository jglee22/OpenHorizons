...
public class CombatTestManager : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        GUILayout.Label("=== 전투 시스템 테스트 ===");
        GUILayout.Label($"E키: 적 생성 (현재: {enemyCount}마리)");
        GUILayout.Label("Q키: 무기 획득");
        GUILayout.Label("R키: 무기 장착");
        GUILayout.Label("T키: 무기 해제");
        GUILayout.Label("H키: 플레이어 체력 회복");
        GUILayout.Label("");
        GUILayout.Label("=== 조작법 ===");
        ...
        if (playerCombat != null)
        {
            ...
            GUILayout.Label($"체력: {playerCombat.currentHealth:F0}/{playerCombat.maxHealth:F0}");
            GUILayout.Label($"무기: {playerCombat.GetWeaponInfo()}");
        }
        
        // 무기 상태 표시
        GUILayout.Label("");
        GUILayout.Label("=== 무기 상태 ===");
        if (inventoryManager != null)
        {
            int weaponCount = inventoryManager.GetItemCount("Bronze Sword");
            if (weaponCount > 0)
            {
                GUILayout.Label($"인벤토리 무기: Bronze Sword x{weaponCount}");
            }
            else
            {
                GUILayout.Label("인벤토리 무기: 없음");
            }
        }
        else
        {
            GUILayout.Label("인벤토리: 연결 안됨");
        }
